...
**Tasks**:
1. Document all files containing endpoint implementations
2. Map test dependencies to endpoint code
3. Identify shared utilities that can be preserved
...
Remove-Item -Recurse -Force tests/integration/protocols/
Remove-Item -Recurse -Force tests/unit/grpc/
Remove-Item -Force tests/unit/interfaces/mcp.test.ts

# Remove API contract tests
Remove-Item -Force tests/integration/protocols/api-contracts.test.ts

# Remove endpoint-specific E2E tests (preserve core E2E tests)
# Review and selectively remove from tests/e2e/
```
...
**Tasks**:
```powershell
# Remove protocol buffer schema
Remove-Item -Force proto/folder-mcp.proto

# Remove all generated code
Remove-Item -Recurse -Force src/generated/

# Update proto generation script
# Edit scripts/generate-proto-types.js or remove if no longer needed
```
...
Remove-Item -Recurse -Force src/grpc/services/

# Clean gRPC server registration code
# Edit src/grpc/server.ts to remove service registrations
```
...
Remove-Item -Recurse -Force src/mcp/handlers/
Remove-Item -Recurse -Force src/interfaces/mcp/handlers/

# Clean MCP server tool registrations
# Edit MCP server files to remove tool registrations
```
...
**Tasks**:
1. Update transport factory to remove endpoint-specific routing
2. Clean typed transport interfaces
3. Remove endpoint-specific transport utilities
4. Preserve core transport framework
...
**Tasks**:
1. Remove endpoint-specific configuration options
2. Preserve core transport and server configurations
...
Remove-Item -Force src/interfaces/cli/commands/test-transport.ts -ErrorAction SilentlyContinue
Remove-Item -Force src/interfaces/cli/commands/test-grpc.ts -ErrorAction SilentlyContinue
...
**Tasks**:
1. Identify dependencies only used for endpoints
