...
- [ ] Create `src/config/manager.ts` as the central configuration orchestrator
- [ ] Extend existing `ConfigFactory` to support new configuration sources
- [ ] Add configuration source priority enum: `DEFAULT < SYSTEM < USER < ENV < RUNTIME`
- [ ] Implement configuration source tracking for debugging
...
- [ ] Create system config loader in `src/config/loaders/system.ts`
- [ ] Check `/etc/folder-mcp/config.yaml` on Unix systems
- [ ] Check `%ProgramData%\folder-mcp\config.yaml` on Windows
- [ ] Add permission handling for system directories
- [ ] Gracefully handle missing system config

#### **1.3 Implement Configuration Profiles**
- [ ] Create `src/config/profiles.ts` for profile management
- [ ] Add profile selection logic (development, staging, production)
- [ ] Support `--profile` CLI argument
- [ ] Allow `FOLDER_MCP_PROFILE` environment variable
...
- [ ] Implement `src/config/watcher.ts` using chokidar
- [ ] Watch configuration files for changes
- [ ] Debounce rapid changes (500ms)
- [ ] Emit configuration change events
- [ ] Support selective reload (only changed sections)
- [ ] Add `--watch-config` flag for development

#### **1.5 Enhance Configuration Validation**
- [ ] Extend existing validation in `src/config/schema.ts`
- [ ] Add cross-field validation support
- [ ] Implement warning-level validations (non-breaking)
- [ ] Create validation context with helpful suggestions
- [ ] Add async validation for external resources

#### **1.6 Implement Smart Defaults**
- [ ] Create `src/config/defaults.ts` with intelligent defaults
- [ ] Detect system capabilities and adjust defaults
- [ ] Use performance tier to set optimal values
- [ ] Provide different defaults per profile
