...
#### Git Workflow Tasks
1. **Continue on current branch `phase-9-sprint-2`**
   - ✅ Draft PR created: https://github.com/okets/folder-mcp/pull/45
   - Use sprint completion commit pattern after sprint done
   - Update PR description with sprint progress

#### Implementation Tasks
2. **Add Express server to daemon (port 3002)**
   - Install express, cors, helmet for security
   - Create `src/daemon/rest/server.ts` 
   - Initialize in daemon startup alongside WebSocket

3. **Keep WebSocket untouched (port 3001 for TUI)**
   - No changes to existing WebSocket implementation
   - Maintain TUI functionality completely

4. **Implement basic REST endpoints**
...
6. **Document REST API structure**
   - Create OpenAPI specification
   - Document endpoint patterns
   - Establish REST conventions

7. **Sprint completion commit**
   - Commit with format: `feat: complete Phase 9 Sprint 1 - REST Foundation`
   - Include all tasks completed and tests passing
...
#### Git Workflow Tasks
1. **Final epic completion commit**
   - Commit with format: `feat: complete Phase 9 Sprint 9 - Legacy cleanup and epic completion`
   - Include summary of all 9 sprints completed
   - Mark PR as ready for review
   - Add comprehensive Co-Authored-By: Claude line

#### Implementation Tasks
2. **Identify and remove legacy single-folder code**
