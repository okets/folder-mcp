...
1. In src/domain/content/pdf-chunking.ts around lines 480 to 484, the code currently imports fs then calls the synchronous fs.readFileSync inside a Promise which blocks the event loop; replace that with the asynchronous fs.promises.readFile (or await an imported fs.promises) to read the file buffer without blocking, then pass the resulting buffer to pdfParser.parseBuffer, and ensure any read or parse errors are properly propagated to reject the outer Promise.

2. In src/domain/content/pdf-chunking.ts around lines 148-149, the code only checks if block is falsy but then accesses block.text which may be undefined; change the guard to verify block.text exists and is a string (e.g., if (!block || !block.text) continue) or treat missing text as empty before computing tokens, and compute blockTokens from block.text.length safely (e.g., use block.text?.length ?? 0) so you avoid a potential null/undefined reference.

3. In src/domain/content/pdf-chunking.ts around lines 449 to 454, the coordinate filtering currently uses textItem.x < pdfParams.x - 1 and textItem.y < pdfParams.y - 1 which incorrectly shifts the boundary and only checks one side; remove the arbitrary "- 1" and change the checks to validate inclusive bounds (e.g., if pdfParams.x !== undefined && textItem.x < pdfParams.x then continue; similarly for y), and if the API supports ranges add corresponding upper-bound checks (e.g., pdfParams.xMax/pdfParams.yMax) to skip items outside the full range.

4. In src/domain/content/pdf-chunking.ts around lines 298 and 374-378, replace all console.error calls in the fallback method with the project's proper logger (use the existing logger/processLogger instance available in this module or accept one if not present), and ensure the messages and any error objects are passed to logger.error instead of console.error; keep the same message text and include error details, and remove console.error usages entirely.


5. In src/domain/content/word-chunking.ts around lines 375 to 381, the code validates startParagraph but only checks that endParagraph is not less than startParagraph—add an upper-bound check for endParagraph as well (and ensure it’s non-negative) by throwing an Error when endParagraph is >= paragraphElements.length or < 0 with a clear message referencing the invalid value and the document paragraph count; keep the existing startParagraph checks and mirror their message style for consistency.

6. In src/domain/content/word-chunking.ts around lines 89-90, the regex /<(p|h[1-6])[^>]*>(.*?)<\/\1>/gs can be exploited for ReDoS on malicious HTML; replace this regex-based extraction with a proper HTML parser (e.g., DOMParser in browser/node's htmlparser2/cheerio) to reliably select <p> and <h1>-<h6> elements and extract their text content, and additionally enforce an input size limit and validate/sanitize htmlContent before parsing (and wrap parsing in error handling/timeouts if using a streaming parser) to prevent processing excessively large or malicious inputs.

7. In src/domain/content/word-chunking.ts around lines 269-270, the code uses fullText.indexOf(text) to compute start/end offsets which is unreliable when the same substring appears multiple times or when whitespace is normalized; instead maintain a running offset while assembling chunks: set startOffset to the current runningOffset, compute endOffset as startOffset + text.length, then advance runningOffset by text.length plus any separator characters you add (e.g., space/newline), and use those offsets directly for the chunk; also handle any normalization by ensuring the assembled fullText and offsets are kept in sync so indexOf is never needed.

8. In src/domain/content/word-chunking.ts around lines 342 to 363 (duplicate of logic in parseWordStructure at ~line 89), the HTML element extraction and text-unescaping regex is duplicated; extract this logic into a single private method (e.g., private parseHtmlElements(htmlContent: string): string[]) that runs the elementRegex, strips nested tags and unescapes entities (or delegates the unescape to a small private helper like extractTextFromHtml), return an array of text content, and replace the duplicated blocks in both parseWordStructure and extractByParams with calls to that new method to remove duplication and centralize parsing behavior.
